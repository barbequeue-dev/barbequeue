...
readonly class CompletionConfirmationMessageFactory
{
    public function create(DeploymentConfirmation $confirmation): SlackPrivateMessage
    {
        ...
        return new SlackPrivateMessage(
            ...
            $queue->getWorkspace(),
            sprintf(
                'Confirmation required to mark the deployment of `%s` to `%s` as completed.',
            ...
            [
                new SectionBlock(
                    sprintf(
                        'Your confirmation is required to mark a deployment by %s of `%s` to `%s` as completed.',
